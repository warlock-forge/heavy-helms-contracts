...
contract Game {
    ...
    function createPlayer(uint256 randomSeed) public returns (Player memory) {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            uint256 availablePoints = remainingPoints > pointsNeededForRemaining ? 
                                    remainingPoints - pointsNeededForRemaining : 0;
    ...
    function _validateStats(Player memory player) private pure returns (bool) {
        ...
        // Check total
        int16 total = int16(player.strength) + 
                     int16(player.constitution) + 
                     int16(player.agility) + 
                     int16(player.stamina);
